...
2.  Раздел **Настройка**, в котором Вы можете создать скрипты, словари, теги и собственные метрики, также настроить алерты в соответствующих вкладках для маркирования сессий по заданным условиям,
...
При переходе в сессию, можно посмотреть параметры каждой сессии, добавленные теги, саммари и при наличии прослушать аудиозапись сессии.

![](https://raw.githubusercontent.com/mts-ai/wordpulse/main/pictures/2.4_Summary.png)

Также можно посмотреть параметры каждой реплики: информацию о дате и времени реплики, тональности, добавленных к реплике тегах.
...
- наличию/отсутствию слов и фраз (точный и семантический поиск),
...
Проанализированные сессии можно экспортировать в файл формата CSV. Это может понадобиться, если, например, необходимо выгрузить сессии за один день, только с определенным клиентом и / или посмотреть сентимент по сессиям.

## Диспетчер задач

Во вкладке **Диспетчер задач** пользователь может посмотреть выполнение запущенных задач, например, экспорт сессий, а также получить результат выполненной задачи.

![](https://raw.githubusercontent.com/mts-ai/wordpulse/main/pictures/2.5_TaskManager.png)
...
Скрипты служат для проверки диалогов с клиентами на основе ключевых слов, тематик диалога, данных клиента или тональности разговора.
...
- критерии - короткие LLM-промпты и теги, в которые нужно сохранить их результат.
...
Данный раздел позволяет настраивать автоматическое уведомление пользователя по email при превышении пороговых значений по метрикам и интересующему фильтру.

![](https://raw.githubusercontent.com/mts-ai/wordpulse/main/pictures/12_Alerts.png)
